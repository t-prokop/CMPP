...
class ant:
    ...
    def leave_pheromone(self, world):
        world[self.pos[0], self.pos[1], self.mode] += self.pheromone_strenght*(self.food_foraged+1)
    ...
    def make_move(self,world, food_counter, beta = 0.99, full_str_pheromone_time = 50):        
        ...
        # print("left",np.remainder(self.pos + self.rotate_left(), self.shape),sniffing)
            ...
            pheromones = [pheromones[i] for i in range(3) if world[moves[i][0], moves[i][1], 0] != 4]
            moves = [moves[i] for i in range(3) if world[moves[i][0], moves[i][1], 0] != 4]
            # print(moves)
            # print(pheromones)
            if len(moves) > 0:
